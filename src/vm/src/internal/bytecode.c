...
vm__bc_t *vm__bc_read(const char* in) {
  ...
  for(int i = 0; i < table_size; ++i)
    table[i] = vm__routine_read(fp);

  return vm__bc_create(version, table_size, table);
}
...
vm__bc_routine_t *vm__routine_create(uint16_t id, uint8_t name_size, const char *name,
